...
// --- Text Scramble Animation (Retro Cinema Style) ---
class TextScramble {
  constructor(el) {
    this.el = el;
  ...
  update() {
    ...
    for (let i = 0, n = this.queue.length; i < n; i++) {
      ...
      if (this.frame >= end) {
        ...
        output += `<span class="opacity-50 font-['JetBrains_Mono']">${char}</span>`;
      } else {
        output += from;
      }
    }
...
// Initialize scramble on targeted elements
...
phrases.forEach((el, index) => {
    ...
    // Add intersection observer to trigger when in view, with stagger
